...
# Thus, we will convert NumCompaniesWorked into categorical variable.

employee$NumCompaniesWorked <- as.factor(employee$NumCompaniesWorked)

# Converting categorical variables to factor
fact_employee <- data.frame(sapply(employee[, c(3:5, 8:9, 11:12, 14)], as.factor))
str(fact_employee)
...
corrplot(cormat, method = "number", order = "AOE", type = 'upper')
...
scale_employee <- data.frame(sapply(cont_employee[, -1], scale))

OTL <- data.frame(sapply(scale_employee, function(x) quantile(x,seq(0,1,.01))))
summary(OTL)
...
new_cont_employee <- data.frame(sapply(scale_employee, outlier_capper))

new_cont_employee <- cbind(cont_employee$EmployeeID, new_cont_employee)

names(new_cont_employee)[1] <- "EmployeeID"

# Outliers removed

# Scaling continuous variables

final_employee <- cbind(new_cont_employee, dummy_df)
...
model_1 <- glm(formula = Attrition ~ .,  data = train[,-1], family = "binomial")
...
model_3 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                 YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                 WorkLifeBalance + Leaves + Tot_Hrs + YearsOutside + 
                 YearsWithOtherManager + BusinessTravel.xTravel_Frequently + 
                 BusinessTravel.xTravel_Rarely + Department.xResearch...Development + 
                 Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                 JobRole.xResearch.Director + MaritalStatus.xMarried + MaritalStatus.xSingle + 
                 NumCompaniesWorked.x1 + NumCompaniesWorked.x2 + NumCompaniesWorked.x3 + 
...
summary(model_3)
# AIC = 2053 (Negligible increase by 1)

sort(vif(model_3), decreasing = T)
# BusinessTravel.xTravel_Rarely is least significant with VIF ~4. Removing it.

model_4 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                 YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                 WorkLifeBalance + Leaves + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                 ...
                 NumCompaniesWorked.x2 + NumCompaniesWorked.x3 + NumCompaniesWorked.x4 + 
                 NumCompaniesWorked.x5 + NumCompaniesWorked.x6 + NumCompaniesWorked.x7 + 
                 NumCompaniesWorked.x8 + NumCompaniesWorked.x9, family = "binomial", 
                 data = train[, -1])
...
model_5.1 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                   YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                   WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
...
model_5.2 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                   YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                   WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                   BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                   Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                   JobRole.xResearch.Director + MaritalStatus.xMarried + MaritalStatus.xSingle + 
                   NumCompaniesWorked.x1 + NumCompaniesWorked.x2 + 
                   NumCompaniesWorked.x4 + NumCompaniesWorked.x5 + NumCompaniesWorked.x6 + 
                   NumCompaniesWorked.x7 + NumCompaniesWorked.x8 + NumCompaniesWorked.x9, 
                   family = "binomial", data = train[, -1])
...
model_6.1 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                   YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                   WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                   BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                   Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                   JobRole.xResearch.Director + MaritalStatus.xMarried + MaritalStatus.xSingle + 
                   NumCompaniesWorked.x1 + 
                   NumCompaniesWorked.x4 + NumCompaniesWorked.x5 + NumCompaniesWorked.x6 + 
                   NumCompaniesWorked.x7 + NumCompaniesWorked.x8 + NumCompaniesWorked.x9, 
                 family = "binomial", data = train[, -1])
...
model_6.2 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                   YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                   WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                   BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                   Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                   ...
                   NumCompaniesWorked.x9, family = "binomial", data = train[, 
                                                                            -1])
...
model_7 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                 YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                 WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                 BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                 Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
...
model_8.1 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                   YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                   WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                   BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                   Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                   ...
                   NumCompaniesWorked.x4 + NumCompaniesWorked.x5 + NumCompaniesWorked.x6 + 
                   NumCompaniesWorked.x7 + NumCompaniesWorked.x9, family = "binomial", 
                 data = train[, -1])
...
model_8.2 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                   YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                   WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                   BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                   Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
...
model_9 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                 YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                 WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                 BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                 Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                 MaritalStatus.xSingle + NumCompaniesWorked.x5 + 
                 NumCompaniesWorked.x6 + NumCompaniesWorked.x7 + NumCompaniesWorked.x9, 
                 family = "binomial", data = train[, -1])
...
model_10 <- glm(formula = Attrition ~ Age + TrainingTimesLastYear + YearsSinceLastPromotion + 
                 YearsWithCurrManager + EnvironmentSatisfaction + JobSatisfaction + 
                 WorkLifeBalance + Tot_Hrs + YearsOutside + YearsWithOtherManager + 
                 BusinessTravel.xTravel_Frequently + Department.xResearch...Development + 
                 Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
...
model_12 <- glm(formula = Attrition ~ TrainingTimesLastYear + YearsSinceLastPromotion + 
                  ...
                  Department.xSales + JobRole.xManager + JobRole.xManufacturing.Director + 
                  MaritalStatus.xSingle + NumCompaniesWorked.x5 + 
                  NumCompaniesWorked.x7, family = "binomial", data = train[, -1])
...
prediction <- predict(final_model, type = "response", newdata = test[,-c(1,24)])
...
s <- seq(0.001479, 0.869581, length=1000)
...
for(i in 1:1000) {OUT[i,] <- perform_fn(s[i])}
...
predicted_attrition <- factor(ifelse(prediction >= 0.17, "Yes", "No"))
